...
contract TokenEvents {
    ...
    event TokenFreeze(address indexed user, uint256 value);

    //when a user unfreezes tokens
    event TokenUnfreeze(address indexed user, uint256 value);

    //when a user freezes freely minted tokens
    event FreeMintFreeze(
        address indexed user,
        uint256 value,
        uint256 indexed dapp //0 for ref, increment per external dapp
    );

    //when a user unfreezes freely minted tokens
    event FreeMintUnfreeze(address indexed user, uint256 value);
    ...
    event FounderLock(uint256 hxyAmt, uint256 timestamp);

    //when founder tokens are unfrozen
    event FounderUnlock(uint256 hxyAmt, uint256 timestamp);

    event LiquidityPush(uint256 amountA, uint256 amountB, uint256 liquidity);

    event DividendPush(uint256 hexDivs);
}

//////////////////////////////////////
//////////HEXMONEY TOKEN CONTRACT////////
////////////////////////////////////
contract HEXMONEY is IBEP20, TokenEvents {
    using SafeMath for uint256;
    using SafeBEP20 for HEXMONEY;
    ...
    uint256 public totalHeartsTransformed = 0;
    uint256 public totalHxyTransformed = 0;
    uint256 public totalDividends = 0;
    uint256 public totalLiquidityAdded = 0;
    uint256 public hexLiquidity = 0;
    uint256 public hexDivs = 0;

    //mint / freeze setup
    uint256 public unlockLvl = 0;
    uint256 public founderLockStartTimestamp = 0;
    uint256 public founderLockDayLength = 1825; //5 years (10% released every sixmonths)
    uint256 public founderLockedTokens = 0;
    uint256 private allFounderLocked = 0;

    bool public mintBlock; //disables any more tokens ever being minted once _totalSupply reaches _maxSupply
    uint256 public minFreezeDayLength = 7; // min days to freeze
    uint256 internal daySeconds = 86400; // seconds in a day
    uint256 public totalFrozen = 0;
    mapping(address => uint256) public tokenFrozenBalances; //balance of HXY frozen mapped by user
    uint256 public totalFreeMintFrozen = 0;
    mapping(address => uint256) public freeMintFrozenBalances; //balance of HXY free minted frozen mapped by user

    //tokenomics
    uint256 public _maxSupply = 10000000000000000; // max supply @ 60M
    uint256 internal _totalSupply;
    string public constant name = "H Binance Smart Chain";
    ...
    function mintInitialTokens(uint256 v2Supply) internal synchronized {
    ...
    function founderLock(uint256 tokens) internal {
    ...
    function unlock() public onlyAdmins synchronized {
        uint256 sixMonths = founderLockDayLength / 10;
        require(unlockLvl < 10, "token unlock complete");
        require(
            founderLockStartTimestamp.add(sixMonths.mul(daySeconds)) <= now,
            "tokens cannot be unfrozen yet"
        ); //must be at least over 6 months
        uint256 value = allFounderLocked / 10;
        if (
            founderLockStartTimestamp.add((sixMonths).mul(daySeconds)) <= now &&
            unlockLvl == 0
        ) {
            ...
            founderLockStartTimestamp.add((sixMonths * 2).mul(daySeconds)) <=
            now &&
            unlockLvl == 1
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 3).mul(daySeconds)) <=
            now &&
            unlockLvl == 2
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 4).mul(daySeconds)) <=
            now &&
            unlockLvl == 3
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 5).mul(daySeconds)) <=
            now &&
            unlockLvl == 4
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 6).mul(daySeconds)) <=
            now &&
            unlockLvl == 5
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 7).mul(daySeconds)) <=
            now &&
            unlockLvl == 6
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 8).mul(daySeconds)) <=
            now &&
            unlockLvl == 7
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 9).mul(daySeconds)) <=
            now &&
            unlockLvl == 8
        ) {
            unlockLvl++;
            ...
            founderLockStartTimestamp.add((sixMonths * 10).mul(daySeconds)) <=
            now &&
            unlockLvl == 9
        ) {
            unlockLvl++;
    ...
    function FreezeTokens(uint256 amt) public {
        ...
        //update balances
        tokenFrozenBalances[msg.sender] = tokenFrozenBalances[msg.sender].add(
            amt
        );
    ...
    function UnfreezeTokens() public synchronized {
        require(
            tokenFrozenBalances[msg.sender] > 0,
            "Error: unsufficient frozen balance"
        ); //ensure user has enough frozen funds
        require(
            isFreezeFinished(msg.sender),
            "tokens cannot be unfrozen yet. min 7 day freeze"
        );
        uint256 amt = tokenFrozenBalances[msg.sender];
    ...
    function calcFreezingRewards(address _user) public view returns (uint256) {
        return (tokenFrozenBalances[_user].div(1000) *
            (minFreezeDayLength + daysPastMinFreezeTime(_user)));
    }
        ...
        returns (uint256)
    {
        ...
        uint256 daysPast = now.sub(frozen[_user].freezeStartTimestamp).div(
            daySeconds
        );
    ...
    function FreezeFreeMint(
        uint256 amt,
        address user,
        uint256 dapp
    ) public onlyMinters synchronized {
        require(amt > 0, "zero input");
        if (!mintBlock) {
            //mint tokens
            ...
            //update balances
            freeMintFrozenBalances[user] = freeMintFrozenBalances[user].add(
                amt
            );
            totalFrozen = totalFrozen.add(amt);
    ...
    function FreezeRefFreeMint(uint256 amt, address ref) internal {
        require(amt > 0, "zero input");
        if (!mintBlock) {
            //mint tokens
            uint256 t = totalSupply();
    ...
    //unfreeze HXY tokens from contract
    function UnfreezeFreeMint() public synchronized {
        require(
        ...
        require(
            mintBlock,
            "tokens cannot be unfrozen yet. max supply not yet reached"
        );
        //update values
        uint256 amt = freeMintFrozenBalances[msg.sender];
        freeMintFrozenBalances[msg.sender] = 0;
        totalFrozen = totalFrozen.sub(amt);
    ...
    //mint HXY to address
    function freeMintHXY(uint256 value, address minter) internal {
        uint256 amt = value;
    ...
    function transformHEX(
        uint256 hearts,
        address ref //Approval needed
    ) public synchronized {
        require(roomActive, "transforms not yet active");
        require(hearts >= 100, "value too low");
        require(
            hexInterface.transferFrom(msg.sender, address(this), hearts),
            "Transfer failed"
        ...
        //get HXY price
        (uint256 reserve0, uint256 reserve1, ) = uniPairInterface.getReserves();
        uint256 hxy = uniV2Router.quote(hearts, reserve0, reserve1);
        ...
        require(
            totalHxyTransformed <= 3000000000000000,
            "transform threshold breached"
        ); //remaining for interest and free mint
        _mint(msg.sender, hxy);
        emit Transform(hearts, hxy, msg.sender);
    }

    function pushLiquidity() public synchronized {
        require(hexLiquidity > 1000, "nothing to add");
        //get price
        (uint256 reserve0, uint256 reserve1, ) = uniPairInterface.getReserves();
        uint256 hxy = uniV2Router.quote(hexLiquidity, reserve0, reserve1);
        _mint(address(this), hxy);
        //approve
        this.safeApprove(routerAddress, hxy);
        require(
            hexInterface.approve(routerAddress, hexLiquidity),
            "could not approve"
        );
        //add liquidity
        (uint256 amountA, uint256 amountB, uint256 liquidity) = uniV2Router
            .addLiquidity(
            hexAddress,
            address(this),
            hexLiquidity,
    ...
    function pushDivs() public synchronized {
    ...
    function setMultiSig(address _multisig) public onlyAdmins onlyOnceMultisig {
        multisig = _multisig;
    }
    ...
    function addMinter(address minter) public onlyMultisig returns (bool) {
        minters[minter] = true;
    ...
    function transformActivate() public onlyMultisig onlyOnceTransform {
        roomActive = true;
    }

    function setExchange(address exchange) public onlyMultisig {
        uniHEXHXY = exchange;
        uniPairInterface = IUniswapV2Pair(uniHEXHXY);
    }

    function setV2Router(address router) public onlyMultisig {
    ...
    function totalFrozenTokenBalance() public view returns (uint256) {
        return totalFrozen;
    }
    ...
    //
    function isFreezeFinished(address _user) public view returns (bool) {
        if (frozen[_user].freezeStartTimestamp == 0) {
            return false;
        } else {
            return
                frozen[_user].freezeStartTimestamp.add(
                    (minFreezeDayLength).mul(daySeconds)
                ) <= now;
        }
    }
